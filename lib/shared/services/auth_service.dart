...
class AuthService extends ChangeNotifier {
  ...
  Future<void> _initializeAuthState() async {
    try {
      ...
      // On web, check for redirect result first (for Google Sign-In redirect flow)
      if (kIsWeb) {
        try {
          ...
          if (redirectResult.user != null) {
            if (kDebugMode) {
              print(
                  '[AuthService][_initializeAuthState] Redirect result found: ${redirectResult.user?.email}');
            }
            // User signed in via redirect, process it
            await _loadUserFromFirestore(redirectResult.user!.uid);
            _loggedIn = true;
            await _determineStage(redirectResult.user!.uid);
            notifyListeners();
          ...
          if (kDebugMode) {
            print(
                '[AuthService][_initializeAuthState] No redirect result or error: $e');
          }
        }
      }
  ...
  Future<void> signInWithGoogle({String role = 'user'}) async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('[AuthService] Unexpected error in Google Sign-In: $e');
      }
